...
export default async function PlaylistsPage() {
  ...
  const playlists = listPlaylists().map((p) => ({ ...p, deviceCount: devices.filter((d) => d.playlistId === p.id).length }));
  return (
    <>
      <PageHeader title="Playlists">
        <Link href="/playlists/new" className="rounded-md bg-neutral-900 px-3 py-1.5 text-sm font-medium text-white hover:bg-neutral-700">
          New playlist
        </Link>
      </PageHeader>
      <Card>
              ...
              {playlists.map((p) => (
                <tr key={p.id}>
                  <td className="py-2.5 pr-3">
                    <Link href={`/playlists/${p.id}`} className="font-medium text-neutral-900 underline-offset-2 hover:underline">
                      {p.name}
                    </Link>
                  </td>
                  <td className="py-2.5 pr-3 text-neutral-600">{p.itemCount}</td>
                  <td className="py-2.5 pr-3 text-neutral-600">{p.totalSeconds ? duration(p.totalSeconds) : "—"}</td>
