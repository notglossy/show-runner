...
export async function GET(req: NextRequest, { params }: RouteContext<"/device/[deviceId]">) {
  ...
  try {
    ...
    if (err instanceof ApiError && err.status === 404) return html(renderPlainPage("Unknown device", deviceId), 404);
    if (err instanceof ApiError) return html(renderPlainPage("Not authorized", "This display needs to register again."), 401);
    throw err;
  }
