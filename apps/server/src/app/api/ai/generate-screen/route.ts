...
export const POST = route(async (req) => {
  ...
  if (!config.AI_API_KEY) {
    throw new ApiError(503, "not_configured", "AI generation is not configured: set AI_API_KEY (and optionally AI_BASE_URL, AI_MODEL).");
  }
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        await generateScreen(request, { baseUrl: config.AI_BASE_URL, apiKey: config.AI_API_KEY!, model: config.AI_MODEL }, emit, {
          signal: req.signal,
        });
      } catch (err) {
        if (!req.signal.aborted) {
          console.error("[ai] generation failed", err);
          emit({ type: "error", message: err instanceof AiProviderError || err instanceof Error ? err.message : String(err) });
        }
      } finally {
        try {
          controller.close();
