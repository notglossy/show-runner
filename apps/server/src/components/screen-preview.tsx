...
export function ScreenPreview({
  ...
  useEffect(() => {
    function onMessage(event: MessageEvent) {
      ...
      logs.current = [...logs.current, { level: event.data.level, message: event.data.message, line: event.data.line }].slice(-20);
      onLogs?.(logs.current);
    }
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex flex-wrap items-center gap-2 text-xs text-neutral-600">
        <span>Data:</span>
        <Button size="sm" variant={source === "sample" ? "primary" : "secondary"} onClick={() => setSource("sample")}>
          Sample
        </Button>
        <Button size="sm" variant={source === "live" ? "primary" : "secondary"} onClick={() => setSource("live")}>
          Live
        </Button>
      ...
      <div ref={box} className="relative w-full overflow-hidden rounded-md border border-neutral-300 bg-black" style={{ aspectRatio: `${CANVAS_W} / ${CANVAS_H}` }}>
        <iframe
          ref={frame}
