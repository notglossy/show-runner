...
export function PlaylistEditor({
  ...
  const [savedJson, setSavedJson] = useState(() =>
    JSON.stringify({
      name: playlist?.name ?? '',
      items:
        playlist?.items.map(({ screenId, dwellSeconds }) => ({ screenId, dwellSeconds })) ?? [],
    }),
  );
  ...
  const body = {
    name,
    items: items.map(({ screenId, dwellSeconds }) => ({ screenId, dwellSeconds })),
  };
  const dirty = JSON.stringify(body) !== savedJson;
  const total = items.reduce(
    (s, i) => s + (Number.isFinite(i.dwellSeconds) ? i.dwellSeconds : 0),
    0,
  );
  ...
  async function save() {
    ...
    try {
      if (playlist) {
        ...
        const { playlist: created } = await api<{ playlist: PlaylistView }>('/api/playlists', {
          method: 'POST',
          body,
        });
  ...
  async function remove() {
    if (
      !playlist ||
      !window.confirm(
        `Delete playlist "${playlist.name}"? Devices using it will show nothing until reassigned.`,
      )
    )
      return;
    try {
              ...
              {items.map((item, index) => (
                <li key={item.key} className="flex flex-wrap items-center gap-2">
                  ...
                  <select
                    ...
                    value={item.screenId}
                    onChange={(e) => update(item.key, { screenId: e.target.value })}
                  >
                    {screens.map((s) => (
          ...
          <div className="mt-4">
            <Button
              ...
              onClick={() =>
                setItems((list) => [
                  ...list,
                  { key: nextKey++, screenId: screens[0]?.id ?? '', dwellSeconds: 30 },
                ])
              }
