...
export function AiPanel({
  ...
  useEffect(() => {
    ...
    api<AiConfig>("/api/ai/config")
      ...
      .catch((err) => !cancelled && setError(err instanceof ApiClientError ? err.detail : String(err)));
    return () => {
      cancelled = true;
      abort.current?.abort();
    };
    ...
    try {
      const res = await fetch("/api/ai/generate-screen", {
        ...
        body: JSON.stringify({
          ...
          previewErrors: basedOnCurrent && includeErrors && previewErrors.length ? previewErrors.slice(0, 20).map((e) => e.slice(0, 1000)) : undefined,
        }),
      });
      if (!res.ok || !res.body) {
      ...
      for (;;) {
        ...
        while ((nl = buffer.indexOf("\n")) >= 0) {
          ...
          if (ev.type === "status") setStatus(`${ev.message} ${secs}s`);
            ...
            const tokens = ev.usage ? ` · ${ev.usage.promptTokens.toLocaleString()} in / ${ev.usage.completionTokens.toLocaleString()} out tokens` : "";
            setStatus(`Done in ${secs}s with ${ev.model}${ev.attempts > 1 ? ` (${ev.attempts} attempts)` : ""}${tokens}. Review the preview, then Save.`);
    ...
    return (
      <Card title="Generate with AI">
        <p className="text-sm text-neutral-600">
          AI generation is off. Set <code>AI_API_KEY</code> (and optionally <code>AI_BASE_URL</code>, <code>AI_MODEL</code>) on the server to enable it.
        </p>
      </Card>
    );
  }
  ...
  return (
    <Card title="Generate with AI" actions={config && <span className="text-xs text-neutral-500">{config.provider}</span>}>
      <form onSubmit={generate} className="flex flex-col gap-3">
        <textarea
          className={`${inputClass} min-h-24 resize-y`}
          placeholder={basedOnCurrent ? "What should change? e.g. make the clock bigger and move the forecast to the left" : "Describe the screen, e.g. a big analog clock with today's high and low and sunrise/sunset"}
          value={instruction}
          onChange={(e) => setInstruction(e.target.value)}
          onKeyDown={(e) => {
            if (e.key === "Enter" && (e.metaKey || e.ctrlKey) && instruction.trim() && !running) void generate(e);
          }}
          disabled={running}
          required
        />
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-neutral-700">
          <label className="flex items-center gap-1.5">
            <input type="checkbox" checked={basedOnCurrent} onChange={(e) => setBasedOnCurrent(e.target.checked)} disabled={running} />
            Based on current screen
          </label>
          {basedOnCurrent && previewErrors.length > 0 && (
            <label className="flex items-center gap-1.5">
              <input type="checkbox" checked={includeErrors} onChange={(e) => setIncludeErrors(e.target.checked)} disabled={running} />
          ...
          <label className="flex min-w-64 flex-1 items-center gap-1.5">
            ...
            <input className={`${inputBase} flex-1 font-mono text-xs`} list="ai-models" value={model} onChange={(e) => rememberModel(e.target.value)} disabled={running} />
        ...
        <div className="flex items-center gap-2">
          <Button type="submit" variant="primary" disabled={running || !instruction.trim() || !config}>
