...
export function validateTemplate(html: string): string[] {
  ...
  if (!trimmed.startsWith("<style")) problems.push("The fragment must start with a <style> element.");
  if (/<!doctype|<html[\s>]|<head[\s>]|<body[\s>]|<meta[\s>]|<link[\s>]|<title[\s>]/i.test(html)) {
    problems.push("Remove document-level tags (<!doctype>, <html>, <head>, <body>, <meta>, <link>, <title>); output a body fragment only.");
  }
  if (/https?:\/\//i.test(html)) problems.push("Remove every http:// or https:// URL; screens must not load external resources.");
  if (/<script[^>]*\bsrc\s*=/i.test(html)) problems.push("Do not use <script src>; inline the script.");
  if (/\bfetch\s*\(|\bXMLHttpRequest\b|\bimport\s*\(|\blocalStorage\b/.test(html)) {
    problems.push("Do not use fetch, XMLHttpRequest, dynamic import, or localStorage; use kiosk.data instead.");
  }
  if (/\bsetInterval\s*\(/.test(html)) problems.push("Do not use setInterval; use kiosk.on('tick', ...) for per-second updates.");
  ...
  const unknown = [...new Set(boundPaths(html).filter((path) => resolvePath(data, path) === undefined))];
  if (unknown.length) {
    problems.push(`These data-bind paths don't exist in kiosk.data (see section 6): ${unknown.join(", ")}.`);
  }
  return problems;
}
