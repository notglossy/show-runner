...
export async function streamChat(options: ChatOptions): Promise<ChatResult> {
  ...
  const handle = (data: string) => {
    ...
    if (json.error) throw new AiProviderError(json.error.code ?? 502, `AI provider error: ${json.error.message ?? "unknown"}`);
    ...
    if (choice.delta?.reasoning) options.onChunk?.({ type: "reasoning", text: choice.delta.reasoning });
