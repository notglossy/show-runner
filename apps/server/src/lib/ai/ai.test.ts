...
import { AiProviderError, streamChat, type ChatChunk, type ChatOptions, type ChatResult } from "./client";
...
const chunk = (delta: object, extra: object = {}) => `data: ${JSON.stringify({ model: "google/gemini-3.8-flash", choices: [{ delta, finish_reason: null }], ...extra })}`;
...
describe("streamChat", () => {
  ...
  it("parses SSE chunks, skips comments, separates reasoning from content, and reads usage", async () => {
    ...
    expect(result).toEqual<ChatResult>({ content: "Hello world", model: "google/gemini-3.8-flash", finishReason: "stop", usage: { promptTokens: 12, completionTokens: 3 } });
    expect(chunks.map((c) => c.type)).toEqual(["reasoning", "content", "content"]);
    const [url, init] = fetchMock.mock.calls[0] as unknown as [string, RequestInit];
    expect(url).toBe("https://openrouter.ai/api/v1/chat/completions");
    expect((init.headers as Record<string, string>).authorization).toBe("Bearer sk-test");
    expect(JSON.parse(init.body as string)).toMatchObject({ model: "google/gemini-3.8-flash", stream: true });
  });

  it("surfaces HTTP errors with the provider's message", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => Response.json({ error: { message: "not/a-model is not a valid model ID", code: 400 } }, { status: 400 })));
    await expect(streamChat(opts())).rejects.toThrow(/HTTP 400: not\/a-model is not a valid model ID/);
  });

  it("surfaces errors sent inside the stream", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => sseResponse([chunk({ content: "partial" }), "", `data: ${JSON.stringify({ error: { message: "rate limited", code: 429 } })}`, ""])));
...
describe("validateTemplate", () => {
  ...
  it("collects bound paths from both binding attributes", () => {
    expect(boundPaths('<a data-bind="time.hhmm"></a><b data-bind-attr="data-i:weather.current.icon; title:time.date"></b>')).toEqual([
      "time.hhmm",
...
describe("prompt", () => {
  ...
  it("builds new-screen and revise messages", () => {
    ...
    const revise = buildMessages({ instruction: "Make it bigger", currentHtml: "<style></style><p/>", previewErrors: ["TypeError: x"], doc: "DOC" });
...
describe("generateScreen", () => {
  ...
  function fakeChat(replies: Partial<ChatResult>[]) {
    ...
    const chat = vi.fn(async (options: ChatOptions) => {
      ...
      return { content: "", model: "used/model", finishReason: "stop", usage: { promptTokens: 100, completionTokens: 50 }, ...next };
    });
    return { chat, calls };
  }
  ...
  async function run(replies: Partial<ChatResult>[], request = { instruction: "A clock" }) {
    ...
    await generateScreen(request, config, (e) => events.push(e), { chat: chat as unknown as typeof streamChat, doc: "DOC" });
  ...
  it("returns valid output from a single call", async () => {
    ...
    expect(result).toMatchObject({ type: "result", html: GOOD, problems: [], attempts: 1, model: "used/model" });
  ...
  it("asks the model to repair invalid output once, with the problems listed", async () => {
    ...
    const { result, calls } = await run([{ content: bad }, { content: `${FENCE}html\n${GOOD}\n${FENCE}` }], { instruction: "x", model: "picked/model" } as never);
    expect(result).toMatchObject({ type: "result", html: GOOD, problems: [], attempts: 2, usage: { promptTokens: 200, completionTokens: 100 } });
