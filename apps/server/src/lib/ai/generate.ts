...
export type GenerateEvent =
  ...
  | { type: "result"; html: string; problems: string[]; model: string; attempts: number; usage: ChatResult["usage"] }
...
export async function generateScreen(
  ...
  for (let attempt = 1; attempt <= maxRepairs + 1; attempt++) {
    emit({ type: "status", message: attempt === 1 ? `Asking ${model}…` : `Fixing ${attempt === 2 ? "problems" : "remaining problems"} (attempt ${attempt})…` });
    let written = 0;
    let reasoned = 0;
    let lastEmit = 0;
    const reply = await chat({
      ...
      onChunk: (chunk) => {
        ...
        if (now - lastEmit > 250) {
          lastEmit = now;
          emit({ type: "progress", phase: written ? "writing" : "reasoning", characters: written || reasoned });
        }
      },
    });
    ...
    const problems = html === null ? ["The reply did not contain an ```html code block with the screen."] : validateTemplate(html);
    if (reply.finishReason === "length") problems.push("The reply was cut off (output token limit); keep the template more compact.");

    if (problems.length === 0 || attempt === maxRepairs + 1) {
      if (html === null) {
        emit({ type: "error", message: `The model didn't return a screen. Reply started: ${reply.content.slice(0, 200) || "(empty)"}` });
      } else {
        emit({ type: "result", html, problems, model: reply.model ?? model, attempts: attempt, usage });
      }
      return;
    }
