...
import { cardinal, openMeteoUrl, toWeatherData, weatherProvider, type OpenMeteoResponse } from "./weather";
...
describe("weather", () => {
  ...
  it("transforms a real response", () => {
    ...
    expect(data.daily[0]).toMatchObject({ date: "2026-09-13", weekday: "Sunday", weekdayShort: "Sun", high: 85, low: 68 });
  ...
  it("maps WMO codes, including unknown and night variants", () => {
    ...
    expect(describeWeatherCode(95)).toMatchObject({ icon: "thunderstorm", condition: "Thunderstorm" });
  ...
  it("falls back to an unavailable payload when the API fails", async () => {
    ...
    expect(data).toMatchObject({ available: false, error: "Open-Meteo HTTP 503", current: null, daily: [] });
    vi.unstubAllGlobals();
  });
});
...
describe("provider cache", () => {
  ...
  it("caches per device for device-scoped providers", async () => {
    ...
    const p: DataProvider<"d", number> = { key: "d", scope: "device", ttlMs: 1000, fetch: impl, fallback: () => 0 };
