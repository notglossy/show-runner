...
const info = (condition: string, icon: WeatherIcon, emojiDay: string, emojiNight = emojiDay): CodeInfo => ({
  condition,
  icon,
  emojiDay,
  emojiNight,
...
export function describeWeatherCode(code: number, isDay = true) {
  ...
  return { weatherCode: code, condition: c.condition, icon: c.icon, emoji: isDay ? c.emojiDay : c.emojiNight };
}
