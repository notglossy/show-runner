...
const authoringDoc = fs.readFileSync(path.resolve(serverRoot, "../../docs/screen-authoring.md"), "utf8");
...
describe("self-hosted fonts", () => {
  for (const font of KIOSK_FONTS) {
    it(`${font.family}: file, license, and docs are present`, () => {
      ...
      const license = path.join(serverRoot, "public", FONTS_PATH, "licenses", font.file.replace(".woff2", "-OFL.txt"));
  ...
  it("declares every font with font-display: block", () => {
    ...
    expect(css).toContain('font-family:"Space Grotesk";src:url("/kiosk/fonts/space-grotesk.woff2") format("woff2");font-weight:300 700');
