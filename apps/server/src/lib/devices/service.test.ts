...
const reg = (deviceId: string = randomUUID()) =>
  registerDevice(
    { deviceId, model: "Echo_Show_8", androidVersion: "11", appVersion: "0.1.0", screenWidth: 1280, screenHeight: 800 },
    "192.168.1.203",
  );
...
beforeEach(() => {
  if (!findScreen(DEFAULT_SCREEN_ID)) createScreen({ id: DEFAULT_SCREEN_ID, name: "Clock", html: "<p>clock</p>", source: "builtin" });
});
afterEach(() => {
  stopScheduler();
  vi.useRealTimers();
...
describe("claim + assignment", () => {
  it("claims by pairing code, assigns the default screen, and tells the device to navigate", () => {
    ...
    off();
    expect(claimed).toMatchObject({ name: "Kitchen", pairingCode: null, assignmentType: "screen", currentScreenId: DEFAULT_SCREEN_ID });
...
describe("playlist rotation", () => {
  it("rotates through items on their dwell times and wraps around", () => {
    ...
    const a = screen("A");
    const b = screen("B");
    const p = createPlaylist({ name: "AB", items: [{ screenId: a.id, dwellSeconds: 10 }, { screenId: b.id, dwellSeconds: 20 }] });
    const { events, off } = listen(device.id);
    ...
    expect(getDeviceOr404(device.id).currentScreenId).toBe(a.id);
    off();
    expect(events.filter((e) => e.type === "navigate").map((e) => "screenId" in e && e.screenId)).toEqual([a.id, b.id, a.id]);
  });
  ...
  it("re-syncs when items change and unassigns when the playlist is deleted", () => {
    ...
    const a = screen("A2");
    const b = screen("B2");
    const p = createPlaylist({ name: "P", items: [{ screenId: a.id, dwellSeconds: 10 }, { screenId: b.id, dwellSeconds: 10 }] });
    applyAssignment(device.id, { type: "playlist", playlistId: p.id });
    vi.advanceTimersByTime(10_000);
    ...
    deletePlaylist(p.id);
    expect(getDeviceOr404(device.id)).toMatchObject({ assignmentType: "none", currentScreenId: null, playlistId: null });
  });
  ...
  it("manual navigate within a playlist restarts the dwell timer", () => {
    ...
    const b = screen("B3");
    const c = screen("C3");
    const p = createPlaylist({ name: "P3", items: [{ screenId: a.id, dwellSeconds: 10 }, { screenId: b.id, dwellSeconds: 10 }] });
    applyAssignment(device.id, { type: "playlist", playlistId: p.id });
...
describe("heartbeat + logs", () => {
  it("records status and last seen, making the device online", () => {
    ...
    const updated = recordHeartbeat(device, { battery: { level: 90, charging: true }, wifi: { rssi: -60, ssid: "home" } }, "10.0.0.2");
  ...
  it("stores logs newest first, paginates, and rate-limits bursts", () => {
    const { device } = reg();
    const accepted = Array.from({ length: 65 }, (_, i) => appendLog(device, { message: `err ${i}`, level: "error" }, "UA"));
