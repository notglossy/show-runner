...
const ok = (schema: { safeParse: (v: unknown) => { success: boolean } }, v: unknown) => expect(schema.safeParse(v).success).toBe(true);
const bad = (schema: { safeParse: (v: unknown) => { success: boolean } }, v: unknown) => expect(schema.safeParse(v).success).toBe(false);

describe("request schemas", () => {
  it("register", () => {
    const base = { deviceId: "3f2b7c1e-8a4d-4e6f-9b0a-1c2d3e4f5a6b", model: "Echo_Show_8", androidVersion: "11", appVersion: "0.1.0", screenWidth: 1280, screenHeight: 800 };
  ...
  it("heartbeat", () => {
    ...
    ok(S.HeartbeatRequestSchema, { battery: { level: 80, charging: true }, wifi: { rssi: -55, ssid: "home" }, uptimeSeconds: 10 });
  ...
  it("claim trims and uppercases the pairing code", () => {
    const parsed = S.ClaimDeviceRequestSchema.parse({ pairingCode: " abc234 ", name: "  Kitchen " });
  ...
  it("logs query coerces and defaults", () => {
    ...
    expect(S.DeviceLogsQuerySchema.parse({ limit: "20", before: "7" })).toEqual({ limit: 20, before: 7 });
  ...
  it("screens", () => {
    ...
    expect(s).toMatchObject({ name: "Clock", description: "", dataRefreshSeconds: 60, source: "user" });
  ...
  it("playlists", () => {
    ...
    ok(S.CreatePlaylistRequestSchema, { name: "Main", items: [{ screenId: "a", dwellSeconds: 30 }] });
    bad(S.CreatePlaylistRequestSchema, { name: "Main", items: [{ screenId: "a", dwellSeconds: 4 }] });
