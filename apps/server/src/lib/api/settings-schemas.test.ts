...
describe("settings schemas", () => {
  it("accepts partial updates and null to clear", () => {
    ...
    pass({ weatherLatitude: null, weatherLongitude: null, weatherUnits: null, weatherLocationName: null, timezone: null });
    expect(S.UpdateSettingsRequestSchema.parse({ weatherLocationName: "  London  " })).toEqual({ weatherLocationName: "London" });
    expect(S.UpdateSettingsRequestSchema.parse({ weatherUnits: "metric" })).toEqual({ weatherUnits: "metric" });
  });
